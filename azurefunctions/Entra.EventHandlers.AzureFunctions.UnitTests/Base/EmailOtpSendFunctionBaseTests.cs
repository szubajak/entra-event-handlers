...
public class AttributeCollectionStartFunctionBaseTests
{
    private readonly TestAttributeCollectionStartFunctionBase _sut;

    private readonly Fixture _fixture = new();

    private readonly ILogger _logger;
    private readonly IAttributeCollectionStartHandler _handler;
    private readonly IRequestAdapter _requestAdapter;
    private readonly IResponseAdapter _responseAdapter;

    public AttributeCollectionStartFunctionBaseTests()
    {
        _logger = Substitute.For<ILogger>();
        _handler = Substitute.For<IAttributeCollectionStartHandler>();
        _requestAdapter = Substitute.For<IRequestAdapter>();
        _responseAdapter = Substitute.For<IResponseAdapter>();

        _sut = new TestAttributeCollectionStartFunctionBase(_logger, _handler, _requestAdapter, _responseAdapter);
    }
    ...
    public async Task ExecuteAsync_Should_Call_Handler_And_Return_Response()
    {
        ...
        _requestAdapter
            .ReadEventAsync<AttributeCollectionStartEvent>(request)
            .Returns(evt);

        var entraResponse = TestResponses.CreateAttributeCollectionStartResponse();
        var handlerResult = new EntraHandlerResult<AttributeCollectionStartResponse>(entraResponse);
    ...
    public async Task ExecuteAsync_Should_Invoke_OnException_When_Handler_Returns_Exception()
    {
        ...
        var entraResponse = TestResponses.CreateAttributeCollectionStartResponse();
        var handlerResult = new EntraHandlerResult<AttributeCollectionStartResponse>(entraResponse, exception);
